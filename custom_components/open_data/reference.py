...
def parse_reference(value: str, portal_url: str | None = None) -> OpenDataReference:
    """Parse a reasonable CKAN or Socrata reference."""
    raw = value.strip()
    if not raw:
    ...
    if segments[:3] == ["api", "3", "action"]:
        ...
        if action == "package_show":
            ...
            return OpenDataReference(PROVIDER_CKAN, portal, dataset_id, None, dataset_id is None)
...
async def async_resolve_reference(session, reference: OpenDataReference) -> OpenDataReference:
    """Detect a provider when URL shape alone is inconclusive."""
...
def _parse_bare_reference(value: str, portal_url: str | None) -> OpenDataReference:
    ...
    if _CKAN_UUID.fullmatch(value) or re.fullmatch(r"[a-z0-9][a-z0-9_-]+", value, re.IGNORECASE):
        if portal is None:
